...
void ChessBoard::initDefaultSetup(void)
{
	...
	square[A8] = SET_BLACK(ROOK);
	square[B8] = SET_BLACK(KNIGHT);
	square[C8] = SET_BLACK(BISHOP);
	square[D8] = SET_BLACK(QUEEN);
	square[E8] = SET_BLACK(KING);
	square[F8] = SET_BLACK(BISHOP);
	square[G8] = SET_BLACK(KNIGHT);
	square[H8] = SET_BLACK(ROOK);
		...
		if (row < 6)
		{
			...
			if (col < 7)
			{
				...
				if ((target_figure = this->square[target_pos]) != EMPTY)
				{
					if (IS_BLACK(figure) != IS_BLACK(target_figure))
					{
						if (FIGURE(target_figure) == KNIGHT)
							check = true;
					}
				}
			}
